...
st.set_page_config(
    ...
    layout="wide",                          # 페이지 레이아웃('ceneterd' | 'wide')
    initial_sidebar_state="collapsed",           # 사이드바 초기 상태('auto' | 'expanded' | 'collapsed')
        menu_items={
            'Get Help': 'https://docks.streamlit.io/',      #"Get help" 메뉴 링크
...
with col1:
    ...
    if text_input:
        st.success(f"TEXT : {text_input}")
    if text_area:
        st.info(f"TEXT : {text_area}")
    if number_input:
        st.warning(f"TEXT : {number_input}")
...
with col4:
    ...
    #인/아웃 스트림 연결 - 인풋 스트림
    with open('/app/dataSet/학생점수.csv','rb') as f: # rb = read binary
        st.download_button(
            label="CSV 다운로드",
            data=f,
            file_name="학샘점수.csv",
...
with col2:
    ...
    st.dataframe(
    df_tbl, 
    use_container_width=True, # True: 컬럼의 최대너비 적용
    hide_index=True,
    column_config={
...
st.data_editor(df_tbl,num_rows="dynamic") #dynamic : 새로운 행 추가 가능
...
df_chart = pd.DataFrame(np.random.randn(20,3), columns=['A','B','C']) # random(행, 열) - 난수값 도출
# st.write(df_chart)
...
# 프로그래스바
# ------------------------
st.header("7단계 : 프로그래스 바와 스피너") # 스피너 : 프로그램이 작동중인 것을 표시해줌
...
with col2:
    ...
    os.makedirs(SAVE_DIR_PATH, exist_ok=True) #폴더가 없으면 자동생성
    uploaded_file_2 = st.file_uploader("파일을 선택하세요", type=['csv','txt','xlsx'], key="uploader_2")

    if uploaded_file_2 is not None:
        #파일 경로 설정
        save_path = os.path.join(SAVE_DIR_PATH, uploaded_file_2.name) # app/Dataset/filenames
        #파일 저장
